  ...
  <div class="heightP100">
    ...
    <el-row class="marginT10">
      <el-table v-loading="bTableLoading" :data="aTableData" border stripe @row-click="selectedRow"
        ...
        <el-table-column label="当日收益率" align="center" min-width="150px" :resizable="false">
          <template slot-scope="scope">
            <div v-if="scope.row.dayRate > 0" class="red">
              {{scope.row.dayRate === '' ? '--': scope.row.dayRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.dayRate == 0">
              {{scope.row.dayRate === '' ? '--': scope.row.dayRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.dayRate < 0" class="green">
              {{scope.row.dayRate === '' ? '--': scope.row.dayRate|percentageFormat(3)}}
            </div>
          </template>
        </el-table-column>
        <el-table-column label="当月年化收益率" align="center" min-width="150px" :resizable="false">
          <template slot-scope="scope">
            <div v-if="scope.row.profitRate > 0" class="red">
              {{scope.row.profitRate === '' ? '--': scope.row.profitRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.profitRate == 0">
              {{scope.row.profitRate === '' ? '--': scope.row.profitRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.profitRate < 0" class="green">
              {{scope.row.profitRate === '' ? '--': scope.row.profitRate|percentageFormat(3)}}
            </div>
          </template>
        </el-table-column>
        <el-table-column label="历史年化收益率" align="center" min-width="150px" :resizable="false">
          <template slot-scope="scope">
            <div v-if="scope.row.yearRate > 0" class="red">
              {{scope.row.yearRate === '' ? '--': scope.row.yearRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.yearRate == 0">
              {{scope.row.yearRate === '' ? '--': scope.row.yearRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.yearRate < 0" class="green">
              {{scope.row.yearRate === '' ? '--': scope.row.yearRate|percentageFormat(3)}}
            </div>
          </template>
        </el-table-column>
        <el-table-column label="当日换手率" align="center" min-width="150px" :resizable="false">
          <template slot-scope="scope">
            <div v-if="scope.row.turnRate > 0" class="red">
              {{scope.row.turnRate === '' ? '--': scope.row.turnRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.turnRate == 0">
              {{scope.row.turnRate === '' ? '--': scope.row.turnRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.turnRate < 0" class="green">
              {{scope.row.turnRate === '' ? '--': scope.row.turnRate|percentageFormat(3)}}
            </div>
          </template>
        </el-table-column>

        <el-table-column label="平均换手率" align="center" min-width="150px" :resizable="false">
          <template slot-scope="scope">
            <div v-if="scope.row.hisTurnRate > 0" class="red">
              {{scope.row.hisTurnRate === '' ? '--': scope.row.hisTurnRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.hisTurnRate == 0">
              {{scope.row.hisTurnRate === '' ? '--': scope.row.hisTurnRate|percentageFormat(3)}}
            </div>
            <div v-if="scope.row.hisTurnRate < 0" class="green">
              {{scope.row.hisTurnRate === '' ? '--': scope.row.hisTurnRate|percentageFormat(3)}}
            </div>
          </template>
        </el-table-column>
  ...
  export default {
    ...
    methods: {
      ...
      getTableData() {
        ...
        API.accountStatisticsHis(data).then(res => {
      ...
      exportExcelData() {
        axios.get("/api/heart/saas/march/statistics/account/countHisExcel?accountId=" + this.accountId + "&dateBegin=" + this.value6[0] + "&dateEnd=" + this.value6[1], {responseType: 'blob'}).then(response => {
